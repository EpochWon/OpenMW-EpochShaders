...
float calcSpecIntensity(vec3 viewNormal, vec3 viewDir, float shininess, vec3 lightDir)
{
    if (dot(viewNormal, lightDir) > 0.0)
    {
        ...
        if (shininess == 255.0) {
            float specularFactor = pow(NdotH, (shininess) / (8.0 * 3.141592653589793));
            float normalizationFactor = (shininess) / (8.0 * 3.141592653589793);
...
vec3 calcHemisphere(vec3 viewNormal)
{
    float w = 0.5 * (1.0 + dot(vec3(0.0, 1.0, 0.0), viewNormal));
    vec3 upperHemisphereCol = gl_LightModel.ambient.xyz;
    vec3 lowerHemisphereCol = gl_LightModel.ambient.xyz * 0.5;
    vec3 col = (w * upperHemisphereCol + (1.0 - w) * lowerHemisphereCol);
    return col;
}
...
#endif
{
    ...
    ambientLight = calcHemisphere(viewNormal);
    ...
    //ambientLight = shadowing + ambientLight;
#else
